...
export default function ComplaintsPage() {
  ...
  // Filter users based on search term - FIXED: handle numeric IDs and exclude current user
  useEffect(() => {
    if (userSearchTerm.trim()) {
      ...
      const filtered = allUsers.filter(u => {
        ...
        return (u.full_name && u.full_name.toLowerCase().includes(searchLower)) ||
  ...
  const fetchAllUsers = async () => {
    ...
    try {
      ...
      // Try the public users endpoint (without admin prefix)
      try {
      ...
      // If no users found, try to get users through product owners
      if (users.length === 0) {
        try {
          // Try to fetch users from products endpoint
          const productResponse = await fetch('/api/products/?limit=50');
          if (productResponse.ok) {
            ...
            users = Array.from(sellerMap.values());
          }
        } catch (e) {
          console.log('Could not fetch users from products');
        }
      }

      // If still no users, create a fallback with some demo users
      if (users.length === 0) {
        // Try to get user by ID from URL if available
        if (reportedUserId) {
          try {
            const response = await fetch(`/api/users/${reportedUserId}/`);
            if (response.ok) {
              const userData = await response.json();
              users = [userData];
            }
          } catch (e) {
            console.log('Could not fetch specific user');
          }
        }
      }
      ...
      // If we have a reported user ID, try to fetch just that user
      if (reportedUserId) {
        try {
          const response = await fetch(`/api/users/${reportedUserId}/`);
          if (response.ok) {
      ...
      if (response.ok) {
        ...
        if (user && userData.id === user.id) {
          setUserValidationStatus({ 
            valid: false, 
            message: 'You cannot report yourself. Please select another user.',
            type: 'error'
          });
          return false;
        }
        ...
        setUserSearchTerm(userData.full_name || userData.username || userData.email || String(userData.id));
        setUserValidationStatus({ 
          valid: true, 
  ...
  const handleSelectUser = (selectedUserData) => {
    ...
    setUserSearchTerm(selectedUserData.full_name || selectedUserData.username || selectedUserData.email || String(selectedUserData.id));
    setUserValidationStatus({ 
      valid: true, 
  ...
  const handleSubmit = async (e) => {
    ...
    const trimmedId = form.reported_user ? String(form.reported_user).trim() : '';
    if (!trimmedId) {
    ...
    // Check if trying to report self
    if (user && String(trimmedId) === String(user.id)) {
            ...
            >
              ...
              <div>
                        ...
                        {isLoadingUsers ? (
                          ...
                          filteredUsers.map((u) => (
                            ...
                            >
                              ...
                              <span className="text-xs text-slate-400 font-mono">ID: {String(u.id).slice(0, 8)}...</span>
                            </button>
                          ))
                        ) : (
                          <div className="p-4 text-center text-sm text-slate-500">
                            {userSearchTerm.trim() ? 'No users found. Try a different search.' : 'Start typing to search for users...'}
                          </div>
                        )}
                      </div>
                    )}
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Start typing to search for users by name, email, or ID. You cannot report yourself.
                </p>
              </div>
